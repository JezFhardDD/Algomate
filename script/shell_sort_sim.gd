...
# --- SCENE RESOURCES (Using ShellBlock.tscn as per your tree) ---
...
# Iterative Shell Sort State
var gap: int = 0
var shell_i: int = 0
var shell_j: int = 0
var is_shifting: bool = false # Tracks the inner insertion sort phase
...
var ANIM_SPEED: float = 1.2 # Standard speed for Shell Sort
...
func _compile_code(code: String) -> void:
	...
	print("=== Shell Sort Compile Request ===")
...
func _perform_sort_step():
	...
	# Check if gap has reduced to 0 -> Done
	if gap <= 0:
	...
	# Check if outer loop 'i' has finished
	if shell_i >= n:
		gap /= 2
		if gap > 0:
			shell_i = gap
			shell_j = shell_i
			status_label.text = "New Gap: %d" % gap
			timeline_log.append("Reducing gap to %d" % gap)
			_add_code_line("GAP", gap, 0)
		else:
			# Sorting Finished
			_finish_simulation()
		is_sorting = false
	...
	# Perform comparison at current shell_j
	if shell_j >= gap:
		var idx_a = shell_j - gap
		var idx_b = shell_j
		
		# Show pointers
		_update_pointers(idx_a, idx_b)
		
		# Highlight blocks
		if block_nodes[idx_a].has_method("set_highlight"): 
			block_nodes[idx_a].set_highlight(true)
		if block_nodes[idx_b].has_method("set_highlight"): 
			block_nodes[idx_b].set_highlight(true)
		
		# Highlight index labels
		if idx_a < index_labels.size():
			index_labels[idx_a].add_theme_color_override("font_color", Color(1, 1, 0, 1))
		if idx_b < index_labels.size():
			index_labels[idx_b].add_theme_color_override("font_color", Color(1, 1, 0, 1))
		
		comparison_counter += 1
		var val_a = main_array[idx_a]
		var val_b = main_array[idx_b]
		
		status_label.text = "Gap %d: Comparing %d vs %d" % [gap, val_a, val_b]
		timeline_log.append("Gap %d: Comparing indices [%d] and [%d]" % [gap, idx_a, idx_b])
		_add_code_line("COMPARE", idx_a, val_a)
		...
		if val_a > val_b:
			...
			_add_code_line("SWAP", idx_a, val_a)
			
			var temp = main_array[idx_a]
			main_array[idx_a] = main_array[idx_b]
			main_array[idx_b] = temp
			
			var node_a = block_nodes[idx_a]
			var node_b = block_nodes[idx_b]
			block_nodes[idx_a] = node_b
			block_nodes[idx_b] = node_a
			
			await _animate_swap(node_a, node_b)
			
			# Continue checking backwards (Insertion Sort logic)
			shell_j -= gap
		else:
			# In order, stop checking this path
			status_label.text = "Correct order. Moving to next item."
			shell_j = -1 # Break inner loop signal
		
		# Unhighlight blocks
		if block_nodes[idx_a].has_method("set_highlight"): 
			block_nodes[idx_a].set_highlight(false)
		if block_nodes[idx_b].has_method("set_highlight"): 
			block_nodes[idx_b].set_highlight(false)
		
		# Reset index label colors
		if idx_a < index_labels.size():
			index_labels[idx_a].add_theme_color_override("font_color", Color(1, 1, 1, 1))
		if idx_b < index_labels.size():
			index_labels[idx_b].add_theme_color_override("font_color", Color(1, 1, 1, 1))
		
	else:
		# Inner loop finished or broke
		shell_i += 1
		shell_j = shell_i # Reset j for next iteration
...
func _gen_cpp_code() -> String:
	var code = "/* Shell Sort Simulation - Operations Log */\n"
	...
	code += "        for(int i = gap; i < n; i++) {\n"
	code += "            int temp = arr[i];\n"
	code += "            int j;\n"
	code += "            for(j = i; j >= gap && arr[j-gap] > temp; j -= gap) {\n"
	code += "                arr[j] = arr[j-gap];\n"
	code += "            }\n"
	code += "            arr[j] = temp;\n"
	code += "        }\n"
	code += "        cout << \"After gap \" << gap << \": \";\n"
	...
	code += "    printArray(arr, n);\n\n"
	code += "    shellSort(arr, n);\n\n"
	code += "    cout << \"Sorted array: \";\n"
	code += "    printArray(arr, n);\n"
	code += "    return 0;\n"
	code += "}\n"
	code += "/* Complexity: Time O(n^2) worst case | Space O(1) */"
	return code
...
func _gen_python_code() -> String:
	...
	code += "                j -= gap\n"
	code += "            arr[j] = temp\n"
	code += "        print(f'After gap {gap}: ', end='')\n"
...
func _gen_java_code() -> String:
	...
	code += "public class ShellSortSim {\n"
	...
	code += "                arr[j] = temp;\n"
	code += "            }\n"
	code += "            System.out.print(\"After gap \" + gap + \": \");\n"
	code += "            printArray(arr);\n"
	code += "        }\n"
	code += "    }\n\n"
...
func start_tutorial() -> void:
	...
	tutorial_sequence = [
		...
		},
		{
			"node": auto_btn,
			"title": "AUTO SORT",
			"text": "Starts/Pauses the automatic shell sort.",
			"action": "highlight"
		},
		{
			"node": timeline_btn,
