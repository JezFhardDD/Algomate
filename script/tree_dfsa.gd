...
func _gen_cpp_code() -> String:
	var code = "/* DFS Tree Search - Shows each node visited */\n"
	code += "#include <iostream>\n#include <stack>\nusing namespace std;\n\n"
	code += "void printTree(int tree[], int size, bool visited[]) {\n"
	code += "    cout << \"Tree state: [\";\n"
	...
	# DFS implementation with step-by-step output
	code += "    // DFS Implementation - Depth First Search\n"
	code += "    stack<int> s;\n"
	code += "    s.push(0);\n"
	code += "    int step = 1;\n\n"
	code += "    cout << \"=== DFS TRAVERSAL STEPS ===\" << endl << endl;\n\n"
	code += "    while(!s.empty()) {\n"
	code += "        int curr = s.top();\n"
	code += "        s.pop();\n"
	code += "        \n"
	...
	code += "        }\n"
	code += "        \n"
	code += "        // Push right then left for DFS order (so left is processed first)\n"
	code += "        int right = 2*curr + 2;\n"
	code += "        int left = 2*curr + 1;\n"
	code += "        \n"
	code += "        if(right < size && !visited[right]) {\n"
	code += "            cout << \"   Pushing right child: node \" << right << \" (value \" << tree[right] << \")\" << endl;\n"
	code += "            s.push(right);\n"
	code += "        }\n"
	code += "        if(left < size && !visited[left]) {\n"
	code += "            cout << \"   Pushing left child: node \" << left << \" (value \" << tree[left] << \")\" << endl;\n"
	code += "            s.push(left);\n"
	code += "        }\n"
	code += "        cout << endl;\n"
	code += "    }\n\n"
...
func _gen_python_code() -> String:
	...
	code += "stack = [0]\n"
	code += "step = 1\n\n"
	code += "print(\"=== DFS TRAVERSAL STEPS ===\\n\")\n\n"
	code += "while stack:\n"
	code += "    curr = stack.pop()\n"
	code += "    \n"
	...
	code += "        print(f\"   Pushing right child: node {right} (value {tree[right]})\")\n"
	code += "        stack.append(right)\n"
	code += "    if left < size and not visited[left]:\n"
	code += "        print(f\"   Pushing left child: node {left} (value {tree[left]})\")\n"
...
func _gen_java_code() -> String:
	...
	code += "        stack.push(0);\n"
	code += "        int step = 1;\n\n"
	code += "        System.out.println(\"=== DFS TRAVERSAL STEPS ===\\n\");\n\n"
	...
	code += "            \n"
	code += "            if(right < size && !visited[right]) {\n"
	code += "                System.out.println(\"   Pushing right child: node \" + right + \" (value \" + tree[right] + \")\");\n"
	code += "                stack.push(right);\n"
	code += "            }\n"
	code += "            if(left < size && !visited[left]) {\n"
	code += "                System.out.println(\"   Pushing left child: node \" + left + \" (value \" + tree[left] + \")\");\n"
...
func _gen_c_code() -> String:
	...
	# DFS Implementation with array stack
	code += "    // DFS Implementation - Depth First Search\n"
	code += "    int stack[100];\n"
	code += "    int top = -1;\n"
	code += "    stack[++top] = 0;\n"
	code += "    int step = 1;\n\n"
	code += "    printf(\"=== DFS TRAVERSAL STEPS ===\\n\\n\");\n\n"
	code += "    while(top >= 0) {\n"
	...
	code += "        \n"
	code += "        if(right < size && !visited[right]) {\n"
	code += "            printf(\"   Pushing right child: node %d (value %d)\\n\", right, tree[right]);\n"
	code += "            stack[++top] = right;\n"
	code += "        }\n"
	code += "        if(left < size && !visited[left]) {\n"
	code += "            printf(\"   Pushing left child: node %d (value %d)\\n\", left, tree[left]);\n"
	code += "            stack[++top] = left;\n"
	code += "        }\n"
	code += "        printf(\"\\n\");\n"
	code += "    }\n\n"
