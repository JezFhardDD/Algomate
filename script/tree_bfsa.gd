...
var code_lines: Array[String] = []  # Track operations for code generation
...
func _ready() -> void:
	...
	if try_again_btn_root:
		try_again_btn_root.pressed.connect(_on_try_again_root_btn_pressed)
...
# ADDED: Helper function to clean up tree animations
func _cleanup_tree_animations():
	for node in tree_nodes:
		if node and node.has_meta("pulse_tween"):
...
# =======================================================
# ASSESSMENT COMPLETION (UPDATED with DB)
# =======================================================
...
func _end_assessment(reason: String):
	...
	# ADDED: Record attempt in database (ALWAYS)
	...
	if reason == "found":
		...
		if grade["passed"]:
			# ADDED: Complete level and get coins from database
...
func _gen_cpp_code() -> String:
	var code = "/* BFS Tree Search - Shows each node visited */\n"
	code += "#include <iostream>\n#include <queue>\n#include <vector>\nusing namespace std;\n\n"
	code += "void printTree(int tree[], int size, vector<bool>& visited) {\n"
	code += "    cout << \"[\";\n"
	...
	code += "    int target = %d;\n" % target_value
	code += "    int size = %d;\n\n" % main_array.size()
	code += "    cout << \"Initial tree (all nodes hidden): \";\n"
	code += "    vector<bool> visited(7, false);\n"
	code += "    printTree(tree, size, visited);\n"
	code += "    cout << \"Searching for target: \" << target << endl << endl;\n\n"
	
	# BFS implementation with step-by-step output
	code += "    // BFS Implementation - Level Order Traversal\n"
	code += "    queue<int> q;\n"
	code += "    q.push(0);\n"
	code += "    visited[0] = true;\n"
	code += "    int step = 1;\n\n"
	code += "    cout << \"=== BFS TRAVERSAL STEPS ===\" << endl << endl;\n\n"
	code += "    while(!q.empty()) {\n"
	code += "        int curr = q.front();\n"
	code += "        q.pop();\n"
	code += "        \n"
	code += "        cout << \"Step \" << step++ << \": Visiting node \" << curr << \" → Value = \" << tree[curr] << endl;\n"
	code += "        cout << \"   After visiting node \" << curr << \": \";\n"
	code += "        printTree(tree, size, visited);\n"
	code += "        \n"
	code += "        if(tree[curr] == target) {\n"
	code += "            cout << \"\\n✓ TARGET FOUND at node \" << curr << \"! ✓\" << endl;\n"
	code += "            break;\n"
	code += "        }\n"
	code += "        \n"
	...
	code += "            cout << \"   Enqueuing left child: node \" << left << \" (value \" << tree[left] << \")\" << endl;\n"
	code += "            q.push(left);\n"
	code += "            visited[left] = true;\n"
	code += "        }\n"
	code += "        if(right < size && !visited[right]) {\n"
	code += "            cout << \"   Enqueuing right child: node \" << right << \" (value \" << tree[right] << \")\" << endl;\n"
	code += "            q.push(right);\n"
	...
	code += "    cout << \"Nodes visited: \";\n"
	code += "    for(int i = 0; i < size; i++) {\n"
	code += "        if(visited[i]) cout << i << \" \";\n"
	code += "    }\n"
	code += "    cout << endl;\n\n"
	code += "    return 0;\n}"
	return code
...
func _gen_python_code() -> String:
	...
	code += "def print_tree(tree, visited):\n"
	code += "    print('[', end='')\n"
	code += "    for i in range(len(tree)):\n"
	code += "        if visited[i]:\n"
	code += "            print(tree[i], end='')\n"
	code += "        else:\n"
	code += "            print('?', end='')\n"
	code += "        if i < len(tree) - 1:\n"
	code += "            print(', ', end='')\n"
	code += "    print(']')\n\n"
	code += "tree = ["
	...
	code += "target = %d\n" % target_value
	code += "size = %d\n\n" % main_array.size()
	code += "print(\"Initial tree (all nodes hidden): \", end='')\n"
	code += "visited = [False] * 7\n"
	code += "print_tree(tree, visited)\n"
	...
	code += "visited[0] = True\n"
	code += "step = 1\n\n"
	code += "print(\"=== BFS TRAVERSAL STEPS ===\\n\")\n\n"
	code += "while queue:\n"
	code += "    curr = queue.popleft()\n"
	code += "    \n"
	code += "    print(f\"Step {step}: Visiting node {curr} → Value = {tree[curr]}\")\n"
	code += "    step += 1\n"
	code += "    print(f\"   After visiting node {curr}: \", end='')\n"
	code += "    print_tree(tree, visited)\n"
	code += "    \n"
	code += "    if tree[curr] == target:\n"
	code += "        print(f\"\\n✓ TARGET FOUND at node {curr}! ✓\")\n"
	code += "        break\n"
	code += "    \n"
	code += "    left = 2*curr + 1\n"
	code += "    right = 2*curr + 2\n"
	...
	code += "        print(f\"   Enqueuing left child: node {left} (value {tree[left]})\")\n"
	code += "        queue.append(left)\n"
	code += "        visited[left] = True\n"
	code += "    if right < size and not visited[right]:\n"
	code += "        print(f\"   Enqueuing right child: node {right} (value {tree[right]})\")\n"
	code += "        queue.append(right)\n"
	code += "        visited[right] = True\n"
	code += "    print()\n\n"
	code += "print(\"\\n=== BFS TRAVERSAL COMPLETE ===\")\n"
	code += "print(f\"Nodes visited: \", end='')\n"
	code += "for i in range(size):\n"
	code += "    if visited[i]:\n"
	code += "        print(i, end=' ')\n"
	code += "print()"
	return code
...
func _gen_java_code() -> String:
	...
	code += "public class BFSTreeSearch {\n"
	code += "    static void printTree(int[] tree, boolean[] visited, int size) {\n"
	...
	code += "        int target = %d;\n" % target_value
	code += "        int size = %d;\n\n" % main_array.size()
	code += "        System.out.print(\"Initial tree (all nodes hidden): \");\n"
	code += "        boolean[] visited = new boolean[7];\n"
	code += "        printTree(tree, visited, size);\n"
	...
	code += "        visited[0] = true;\n"
	code += "        int step = 1;\n\n"
	code += "        System.out.println(\"=== BFS TRAVERSAL STEPS ===\\n\");\n\n"
	code += "        while(!queue.isEmpty()) {\n"
	code += "            int curr = queue.poll();\n"
	code += "            \n"
	code += "            System.out.println(\"Step \" + step++ + \": Visiting node \" + curr + \" → Value = \" + tree[curr]);\n"
	code += "            System.out.print(\"   After visiting node \" + curr + \": \");\n"
	code += "            printTree(tree, visited, size);\n"
	code += "            \n"
	code += "            if(tree[curr] == target) {\n"
	code += "                System.out.println(\"\\n✓ TARGET FOUND at node \" + curr + \"! ✓\");\n"
	...
	code += "            \n"
	code += "            if(left < size && !visited[left]) {\n"
	code += "                System.out.println(\"   Enqueuing left child: node \" + left + \" (value \" + tree[left] + \")\");\n"
	code += "                queue.offer(left);\n"
	code += "                visited[left] = true;\n"
	code += "            }\n"
	code += "            if(right < size && !visited[right]) {\n"
	code += "                System.out.println(\"   Enqueuing right child: node \" + right + \" (value \" + tree[right] + \")\");\n"
	code += "                queue.offer(right);\n"
	code += "                visited[right] = true;\n"
	code += "            }\n"
	code += "            System.out.println();\n"
	code += "        }\n\n"
	code += "        System.out.println(\"\\n=== BFS TRAVERSAL COMPLETE ===\");\n"
	code += "        System.out.print(\"Nodes visited: \");\n"
	code += "        for(int i = 0; i < size; i++) {\n"
	code += "            if(visited[i]) System.out.print(i + \" \");\n"
	code += "        }\n"
	code += "        System.out.println();\n"
	code += "    }\n"
	code += "}"
...
func _gen_c_code() -> String:
	...
	code += "void printTree(int tree[], bool visited[], int size) {\n"
	code += "    printf(\"[\");\n"
	code += "    for(int i = 0; i < size; i++) {\n"
	code += "        if(visited[i]) {\n"
	...
	code += "    int target = %d;\n" % target_value
	code += "    int size = %d;\n\n" % main_array.size()
	code += "    printf(\"Initial tree (all nodes hidden): \");\n"
	code += "    bool visited[7] = {false};\n"
	code += "    printTree(tree, visited, size);\n"
	code += "    printf(\"Searching for target: %d\\n\\n\", target);\n\n"
	
	# BFS Implementation with array queue
	code += "    // BFS Implementation - Level Order Traversal\n"
	code += "    int queue[100];\n"
	...
	code += "    visited[0] = true;\n"
	code += "    int step = 1;\n\n"
	code += "    printf(\"=== BFS TRAVERSAL STEPS ===\\n\\n\");\n\n"
	code += "    while(front < rear) {\n"
	code += "        int curr = queue[front++];\n"
	code += "        \n"
	code += "        printf(\"Step %d: Visiting node %d → Value = %d\\n\", step++, curr, tree[curr]);\n"
	code += "        printf(\"   After visiting node %d: \", curr);\n"
	code += "        printTree(tree, visited, size);\n"
	code += "        \n"
	code += "        if(tree[curr] == target) {\n"
	code += "            printf(\"\\n✓ TARGET FOUND at node %d! ✓\\n\", curr);\n"
	code += "            break;\n"
	code += "        }\n"
	code += "        \n"
	code += "        int left = 2*curr + 1;\n"
	...
	code += "        \n"
	code += "        if(left < size && !visited[left]) {\n"
	code += "            printf(\"   Enqueuing left child: node %d (value %d)\\n\", left, tree[left]);\n"
	code += "            queue[rear++] = left;\n"
	code += "            visited[left] = true;\n"
	code += "        }\n"
	code += "        if(right < size && !visited[right]) {\n"
	code += "            printf(\"   Enqueuing right child: node %d (value %d)\\n\", right, tree[right]);\n"
	code += "            queue[rear++] = right;\n"
	code += "            visited[right] = true;\n"
	code += "        }\n"
	code += "        printf(\"\\n\");\n"
	code += "    }\n\n"
	code += "    printf(\"\\n=== BFS TRAVERSAL COMPLETE ===\\n\");\n"
	code += "    printf(\"Nodes visited: \");\n"
	code += "    for(int i = 0; i < size; i++) {\n"
	code += "        if(visited[i]) printf(\"%d \", i);\n"
	code += "    }\n"
	code += "    printf(\"\\n\");\n\n"
	code += "    return 0;\n}"
	return code
