...
var intro_texts = [
	"Welcome to Insertion Sort Assessment!\n\nInsertion Sort builds the final sorted array one element at a time by repeatedly inserting elements into their correct position.",
	"The Algorithm:\n\n1. Start with the second element (index 1)\n2. Compare it with elements before it\n3. Shift larger elements right\n4. Insert the element in its correct position\n5. Move to next unsorted element",
	"Instructions:\n\n• Drag the highlighted block (target) to its correct position in the sorted portion\n• Green feedback = correct insertion position\n• Red feedback = incorrect insertion position\n• Only correct insertions count as good moves",
	"Scoring:\n\n✓ Correct insertion = Good move\n✗ Wrong insertion = Bad move\n\nAccuracy = Good moves / Total moves\n\nUndo/Redo available on Easy & Medium!",
...
func _ready() -> void:
	...
	# Connect simulation confirmation buttons
	if sim_yes_btn:
...
func _get_code_for_language(lang: String, arr_str: String) -> String:
	match lang:
		"cpp": return get_cpp_insertion_code(arr_str)
		"c": return get_c_insertion_code(arr_str)
		"java": return get_java_insertion_code(arr_str)
		"python": return get_python_insertion_code(arr_str)
	return get_cpp_insertion_code(arr_str)
...
func _start_assessment_mode() -> void:
	...
	if difficulty == 1:
		...
		timer_label.show()
		timer_running = false  # Don't start yet — wait for intro to finish
		await get_tree().process_frame
		clock.visible = true
...
func _initialize_with_elements(elements: Array[int]) -> void:
	...
	sorted_array = elements.duplicate()  # ADD THIS!
...
func _safe_connect(node: Node, signal_name: String, method: Callable) -> void:
...
func _process(delta: float) -> void:
	...
	# Update timer label with null check
	if timer_label:
	...
	# Play tik-tak sound when 10 seconds remaining
...
# ==============================================
#   BLOCK DRAG & DROP (INSERTION SORT LOGIC)
# ==============================================
func _on_block_dropped(dropped_block: Control) -> void:
	...
	if is_valid:
		correct_moves += 1
		timeline_log.append(
			"[color=green]✓ Good move: %d moved from index %d to %d[/color]" 
	...
	# Update visual state based on the current array
	_update_current_target()
...
func _validate_insertion_move(old_index: int, new_index: int) -> Dictionary:
	...
	# Get the values
	var moving_value = main_array[old_index]
	var neighbor_value = main_array[new_index]
	
	# Determine direction
	if new_index < old_index:  # Moving LEFT
		# When moving left, the moving element must be SMALLER than the element it's swapping with
		if moving_value >= neighbor_value:
			return {
				"valid": false,
				"message": "Cannot move %d left - it's not smaller than %d" % [moving_value, neighbor_value]
			}
		
		# That's it! For leftward moves, we ONLY check the immediate swap
		# We DON'T check elements beyond the swap because they'll be handled in subsequent moves
		return {
			"valid": true,
			"message": "Good move! Moving %d left" % moving_value
		}
...
func _get_current_target_index() -> int:
	...
	# This is the element that needs to be inserted
...
func _update_current_target() -> void:
	...
	# Get the current target (element to insert)
	var target = _get_current_target_index()
	
	# Reset all highlights and scales
	for i in range(block_nodes.size()):
		...
		if block_nodes[i].has_method("set_sorted_visual"):
			# Elements before target are sorted
			if i < target:
	...
	# Highlight the target element (the one to insert)
	if target < block_nodes.size() and is_instance_valid(block_nodes[target]):
		...
		# Update status label
		if status_label:
			status_label.text = "Insert element %d into sorted portion" % main_array[target]
...
func _end_assessment(reason: String) -> void:
	...
	if reason == "timeout":
		# Timeout = fail, just record attempt
...
# ==============================================
#   AUTO SORT (for lecture mode - kept for compatibility)
# ==============================================
func _on_auto_pressed() -> void:
	...
	undo_btn.disabled = is_auto_playing
...
func _perform_insertion_step() -> void:
	is_sorting = true
	...
	if insert_at != current_index:
		# Remove the key
		var key_val = main_array.pop_at(current_index)
		# Insert at correct position
		main_array.insert(insert_at, key_val)
		
		# Update blocks to match array
		_rebuild_blocks_from_array()
...
func _on_cpp_lang_pressed() -> void: 
	btn_sound.play()
	current_code_language = "cpp"
	_show_cpp_popup()
	
func _on_python_lang_pressed() -> void: 
	btn_sound.play()
	current_code_language = "python"
	_show_cpp_popup()
	
func _on_java_lang_pressed() -> void: 
	btn_sound.play()
	current_code_language = "java"
	_show_cpp_popup()
	
func _on_c_lang_pressed() -> void: 
	btn_sound.play()
	...
	tutorial_sequence = [
		{
			"node": undo_btn,
			"title": "UNDO",
			"text": "Reverts your last insertion.\nAvailable on Easy & Medium only.",
			"action": "highlight"
		},
		{
			"node": redo_btn,
			"title": "REDO",
			"text": "Reapplies an undone insertion.\nAvailable on Easy & Medium only.",
			"action": "highlight"
		},
		{
			"node": timeline_btn,
			"title": "TIMELINE",
			"text": "View a scrollable history of all your insertions.",
			"action": "highlight"
		}
	]
	_show_tutorial_step()
...
# ==============================================
#   CONFIGURATION MODALS (for custom arrays)
# ==============================================
