...
func _get_time_limit() -> float:
	match difficulty:
		1: return 0  # Easy (no timer)
		2: return 90.0     # Medium (15 min)
...
func _start_assessment_mode():
	...
	# Ensure code buttons are hidden at start of new assessment
	if cpp_code_button:
	...
	if difficulty == 1:
		...
		timer_label.show()
		timer_running = false  # Don't start yet — intro hasn't finished
		await get_tree().process_frame
		clock.visible = true
		clock.modulate = Color(1, 1, 1, 1)
...
func _advance_to_next_merge():
	...
	# Only end if actually sorted
	if _check_if_sorted():
		print("Array is sorted! Ending assessment.")
		_end_assessment("sorted")
		return
	
	# Find next valid merge operation
	while true:
		# If left_start has passed the array, increase width and reset
		if left_start >= n:
			current_width *= 2
			left_start = 0
			print("Increased width to: ", current_width)
		
		# If width covers whole array and still not sorted,
		# reset to width=1 and start over (keeps going until sorted)
		if current_width >= n:
			current_width = 1
			left_start = 0
			print("Resetting width to 1 for another pass")
		
		mid_point = min(left_start + current_width - 1, n - 1)
		right_end = min(left_start + 2 * current_width - 1, n - 1)
		
		print("Checking merge: left=", left_start, " mid=", mid_point, " right=", right_end)
		
		# Valid merge: right subarray must exist and have elements
		if mid_point < right_end and mid_point < n - 1:
			# Check if this subarray pair is already sorted
			...
			if already_sorted:
				# Skip this merge, move to next
				left_start += 2 * current_width
				continue
			
			# Valid unsorted merge found
			...
			# No valid merge at this position, skip
			left_start += 2 * current_width
			continue
...
func _finish_current_merge():
	...
	timeline_log.append(
		"[color=green]--- Merge complete ---[/color]"
	)
	
	# Just advance left_start — _advance_to_next_merge handles width increases
	left_start += 2 * current_width
	current_phase = MergePhase.IDLE
...
func _compute_grade() -> Dictionary:
	...
	if total_moves == 0:
		return {
			...
			"coins": 0,  # Will be set by DB if passed
	...
	return {
		...
		"mistake_counter": mistake_counter,
		"time_used": time_used,
		"coins": 0,  # Will be set by DB in _end_assessment if passed
...
func _show_result_popup(result: String, grade_data: Dictionary = {}) -> void:
	...
	if completion_type == "timeout":
		...
		# Double ensure code buttons are hidden on timeout
		if translate_code_btn:
			translate_code_btn.hide()
		if cpp_code_button:
			cpp_code_button.hide()
