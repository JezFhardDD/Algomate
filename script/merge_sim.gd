...
func _compile_code(code: String) -> void:
	...
	print("=== Merge Sort Compile Request ===")
...
func _perform_sort_step():
	...
	if not is_merging_step:
		...
		_add_code_line("MERGE_START", left_start, curr_width)
	...
	if is_merging_step:
		if m_i <= m_mid and m_j <= m_right_end:
			...
			if m_i < index_labels.size():
				index_labels[m_i].add_theme_color_override("font_color", Color(1, 1, 0, 1))
			if m_j < index_labels.size():
				index_labels[m_j].add_theme_color_override("font_color", Color(1, 1, 0, 1))
			...
			if val_left <= val_right:
				...
				if m_i < index_labels.size():
					index_labels[m_i].add_theme_color_override("font_color", Color(1, 1, 1, 1))
				if m_j < index_labels.size():
					index_labels[m_j].add_theme_color_override("font_color", Color(1, 1, 1, 1))
				
				m_i += 1
			else:
				status_label.text = "%d > %d. Moving %d." % [val_left, val_right, val_right]
				timeline_log.append("Moving %d to index %d" % [val_right, m_i])
				_add_code_line("MOVE", m_i, val_right)
				swap_counter += 1
			...
			timeline_log.append("Merge complete for width %d" % curr_width)
			_add_code_line("MERGE_COMPLETE", curr_width, 0)
			_resnap_blocks()
...
func _gen_cpp_code() -> String:
	var code = "/* Merge Sort Simulation - Operations Log */\n"
	...
	code += "    int n1 = mid - left + 1;\n"
	code += "    int n2 = right - mid;\n"
	code += "    int L[n1], R[n2];\n\n"
	code += "    for(int i = 0; i < n1; i++) L[i] = arr[left + i];\n"
	code += "    for(int j = 0; j < n2; j++) R[j] = arr[mid + 1 + j];\n\n"
	code += "    int i = 0, j = 0, k = left;\n"
	code += "    while(i < n1 && j < n2) {\n"
	code += "        if(L[i] <= R[j]) arr[k++] = L[i++];\n"
	code += "        else arr[k++] = R[j++];\n"
	code += "    }\n"
	code += "    while(i < n1) arr[k++] = L[i++];\n"
	code += "    while(j < n2) arr[k++] = R[j++];\n"
	code += "}\n\n"
	code += "void mergeSort(int arr[], int n) {\n"
	code += "    for(int width = 1; width < n; width *= 2) {\n"
	code += "        for(int i = 0; i < n; i += 2*width) {\n"
	code += "            int mid = min(i + width - 1, n-1);\n"
	code += "            int right = min(i + 2*width - 1, n-1);\n"
	code += "            if(mid < right) merge(arr, i, mid, right);\n"
	code += "        }\n"
	code += "        cout << \"After merging width \" << width << \": \";\n"
	...
	code += "    printArray(arr, n);\n\n"
	code += "    mergeSort(arr, n);\n\n"
	code += "    cout << \"Sorted array: \";\n"
	code += "    printArray(arr, n);\n"
	code += "    return 0;\n"
	code += "}\n"
	code += "/* Complexity: Time O(n log n) | Space O(n) */"
	return code
...
func _gen_python_code() -> String:
	...
	code += "    n1 = mid - left + 1\n"
	code += "    n2 = right - mid\n"
	code += "    L = arr[left:mid+1]\n"
	code += "    R = arr[mid+1:right+1]\n\n"
	code += "    i = j = 0\n"
	code += "    k = left\n\n"
	code += "    while i < n1 and j < n2:\n"
	code += "        if L[i] <= R[j]:\n"
	code += "            arr[k] = L[i]\n"
	code += "            i += 1\n"
	code += "        else:\n"
	...
	code += "    width = 1\n"
	code += "    while width < n:\n"
	code += "        for i in range(0, n, 2*width):\n"
	code += "            left = i\n"
	code += "            mid = min(i + width - 1, n-1)\n"
	code += "            right = min(i + 2*width - 1, n-1)\n"
	code += "            if mid < right:\n"
	code += "                merge(arr, left, mid, right)\n"
	code += "        print(f'After merging width {width}: ', end='')\n"
...
func _gen_java_code() -> String:
	...
	code += "public class MergeSortSim {\n"
	...
	code += "    }\n\n"
	code += "    public static void merge(int[] arr, int left, int mid, int right) {\n"
	code += "        int n1 = mid - left + 1;\n"
	code += "        int n2 = right - mid;\n\n"
	code += "        int[] L = new int[n1];\n"
	code += "        int[] R = new int[n2];\n\n"
	code += "        for(int i = 0; i < n1; i++) L[i] = arr[left + i];\n"
	code += "        for(int j = 0; j < n2; j++) R[j] = arr[mid + 1 + j];\n\n"
	code += "        int i = 0, j = 0, k = left;\n"
	code += "        while(i < n1 && j < n2) {\n"
	...
	code += "        for(int width = 1; width < n; width *= 2) {\n"
	code += "            for(int i = 0; i < n; i += 2*width) {\n"
	code += "                int mid = Math.min(i + width - 1, n-1);\n"
	code += "                int right = Math.min(i + 2*width - 1, n-1);\n"
	code += "                if(mid < right) merge(arr, i, mid, right);\n"
	code += "            }\n"
	code += "            System.out.print(\"After merging width \" + width + \": \");\n"
	code += "            printArray(arr);\n"
	code += "        }\n"
	code += "    }\n\n"
...
func _gen_c_code() -> String:
	...
	code += "    for(int width = 1; width < n; width *= 2) {\n"
	code += "        for(int i = 0; i < n; i += 2*width) {\n"
	code += "            int mid = (i + width - 1 < n-1) ? i + width - 1 : n-1;\n"
	code += "            int right = (i + 2*width - 1 < n-1) ? i + 2*width - 1 : n-1;\n"
	code += "            if(mid < right) merge(arr, i, mid, right);\n"
	code += "        }\n"
	code += "        printf(\"After merging width %d: \", width);\n"
	code += "        printArray(arr, n);\n"
	code += "    }\n"
	code += "}\n\n"
	code += "int main() {\n"
