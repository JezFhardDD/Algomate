...
func _ready() -> void:
	...
	if time_up_try and not time_up_try.is_connected("pressed", _on_time_up_try_again_pressed):
		time_up_try.pressed.connect(_on_time_up_try_again_pressed)
	if time_up_back and not time_up_back.is_connected("pressed", _on_time_up_back_pressed):
...
func _start_assessment_mode():
	...
	if difficulty == 1:
		...
		timer_label.show()
		timer_running = false  # Don't start yet — wait for intro
		await get_tree().process_frame
		clock.visible = true
		clock.modulate = Color(1, 1, 1, 1)
...
func _show_result_popup(result: String, grade_data: Dictionary = {}) -> void:
	...
	if result == "PASS":
		...
		if translate_code_btn: translate_code_btn.hide()  # ADD THIS
		if cpp_code_button: cpp_code_button.hide()         # ADD THIS
