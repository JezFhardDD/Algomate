...
func _show_cpp_popup() -> void:
	...
	match current_code_language:
		"cpp": code = get_cpp_binary_code(arr_str, search_target)
		"python": code = get_python_binary_code(arr_str, search_target)
		"java": code = get_java_binary_code(arr_str, search_target)
		"c": code = get_c_binary_code(arr_str, search_target)
...
arr = [%s]
target = %d
print(binary_search(arr, target))""" % [arr, target]

func get_java_binary_code(arr: String, target: int) -> String:
	return """class BinarySearch {
