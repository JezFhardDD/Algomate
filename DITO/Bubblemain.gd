...
var intro_texts = [
	...
	"Instructions:\n\n• Drag adjacent blocks to swap\n• Green = correct swap (reduces inversions)\n• Red = incorrect swap\n• Any adjacent swap is allowed",
...
func _animate_swap(node_a: Control, node_b: Control):
	...
	current_tween.tween_property(node_a, "position", pos_b, ANIM_SPEED)
	current_tween.tween_property(node_b, "position", pos_a, ANIM_SPEED)
...
func _update_pointers(left_idx: int, right_idx: int):
	...
	if left_idx < block_nodes.size() and block_nodes[left_idx]:
		var node = block_nodes[left_idx]
		if ptr_left:
			ptr_left.global_position = node.global_position + Vector2(16, node.size.y + 10)
	
	if right_idx < block_nodes.size() and block_nodes[right_idx]:
