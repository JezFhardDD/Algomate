...
func _gen_cpp_code() -> String:
	var code = "/* Queue Simulation - Operations Log */\n"
	...
	# Enqueue operations
	if waiting_elements.size() > 0:
		code += "    // Enqueue operations\n"
		code += "    for(int i = 0; i < n; i++) {\n"
		code += "        q.push(elements[i]);\n"
		code += "        cout << \"After enqueue \" << elements[i] << \": \";\n"
		code += "        printQueue(q);\n"
		code += "    }\n\n"
	
	# Dequeue operations
	code += "    // Dequeue operations\n"
	code += "    while(!q.empty()) {\n"
	code += "        cout << \"Front element: \" << q.front() << endl;\n"
	code += "        q.pop();\n"
	...
	code += "    return 0;\n"
	code += "}\n"
	code += "/* Complexity: Enqueue/Dequeue O(1) | Space O(n) */"
	return code

func _gen_python_code() -> String:
	var code = "# Queue Simulation - Operations Log\n\n"
	code += "from collections import deque\n\n"
	code += "def print_queue(q):\n"
	code += "    print('[', end='')\n"
	code += "    for i, val in enumerate(q):\n"
	code += "        print(val, end='')\n"
	code += "        if i < len(q) - 1:\n"
	code += "            print(', ', end='')\n"
	code += "    print(']')\n\n"
	...
	if waiting_elements.size() > 0:
		...
		code += "    q.append(val)\n"
		code += "    print(f'After enqueue {val}: ', end='')\n"
		code += "    print_queue(q)\n\n"
	
	code += "# Dequeue operations\n"
	code += "while q:\n"
	code += "    print('Front element:', q[0])\n"
	code += "    q.popleft()\n"
	code += "    print('After dequeue: ', end='')\n"
	code += "    print_queue(q)\n\n"
...
func _gen_java_code() -> String:
	...
	code += "public class QueueSim {\n"
	code += "    public static void printQueue(Queue<Integer> q) {\n"
	code += "        System.out.print(\"[\");\n"
	code += "        Queue<Integer> temp = new LinkedList<>();\n"
	code += "        temp.addAll(q);\n"
	code += "        while(!temp.isEmpty()) {\n"
	code += "            System.out.print(temp.poll());\n"
	...
	if waiting_elements.size() > 0:
		...
		code += "            q.add(val);\n"
		code += "            System.out.print(\"After enqueue \" + val + \": \");\n"
		code += "            printQueue(q);\n"
		code += "        }\n\n"
	
	code += "        // Dequeue operations\n"
	code += "        while(!q.isEmpty()) {\n"
	code += "            System.out.println(\"Front element: \" + q.peek());\n"
	code += "            q.poll();\n"
...
func _gen_c_code() -> String:
	...
	if waiting_elements.size() > 0:
		code += "    // Enqueue operations\n"
		code += "    for(int i = 0; i < n; i++) {\n"
		code += "        enqueue(elements[i]);\n"
		code += "        printf(\"After enqueue %d: \", elements[i]);\n"
		code += "        printQueue();\n"
		code += "    }\n\n"
	
	code += "    // Dequeue operations\n"
	code += "    while(!isEmpty()) {\n"
	code += "        printf(\"Front element: %d\\n\", peek());\n"
	code += "        dequeue();\n"
