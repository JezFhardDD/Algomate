...
func _handle_pos_selection(index: int):
	...
	# START STRONG ANIMATION on this block
	print("Starting animation on block ", index)  # Debug print
	
	# Clear any existing animation on this block
	if block.has_meta("compare_animation"):
		var old_anim = block.get_meta("compare_animation")
		if old_anim and old_anim.is_running():
			old_anim.kill()
	
	# Create a more noticeable animation
	var tween = create_tween().set_loops()  # Infinite loops
	
	# Sequence of effects
	tween.set_parallel(false)
	
	# First, a quick "pop" to acknowledge the tap
	tween.tween_property(block, "scale", Vector2(1.2, 1.2), 0.1)
	tween.tween_property(block, "scale", Vector2(1.15, 1.15), 0.1)
	
	# Then start the pulsing loop
	for i in range(4):  # We'll create a repeating pattern manually
		tween.set_parallel(true)
		tween.tween_property(block, "scale", Vector2(1.2, 1.2), 0.3)
		tween.tween_property(block, "modulate", Color(1, 1, 0.5, 1), 0.3)  # Bright yellow
...
int main() {
	int arr[] = {%s};
	int target = %d;
	int result = interpolationSearch(arr, 6, target);
