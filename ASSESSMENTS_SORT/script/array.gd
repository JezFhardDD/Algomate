extends Control
# --- LABELS & CONTAINERS ---
#@onready var status_label: Label = $MarginContainer/HBoxContainer2/TextureRect/Label  # This is the "Array Size: X/X" label
@onready var array_size_label: Label = $MarginContainer/HBoxContainer2/TextureRect/Label  # Same node, but we'll use it for size display
#@onready var feedback_label: Label = $HBoxContainer/Label  # This might be for feedback messages
...
# These might be nested differently - let's get them safely
...
# NEW: Apply simulation-ended state (only simulate new + timeline enabled)
...
# Change the function declaration to accept any array type
...
# FIX: Tutorial next button - force it interactable inside start_tutorial
func start_tutorial():
...
# FIX: Update label after every insert/delete
func _execute_insert_end(value: int):
	...
	_update_button_states()
	_update_array_size_label()  # FIX: update label
	_update_timeline_display()
...
# ==============================================
#   TUTORIAL FUNCTIONS (Simplified for array simulation)
# ==============================================
...
func _build_walkthrough_steps(lang: String) -> Array:
	...
	# Each step: { "line_range": [start, end], "explanation": "text" }
	# We'll highlight by line index in the generated code
		...
		"cpp":
			...
			for i in range(code_lines.size()):
				...
				if line.begins_with("Initial list"):
					...
					steps.append({
						"lines": [16 + i * 3, 16 + i * 3 + 2],
						"explanation": "[b]Insert at End[/b]\nAppends a new element to the back of the vector.\n[color=green]Result: %s[/color]" % (parts[1] if parts.size() > 1 else "")
					})
				elif line.begins_with("After insert") and "index" in line:
					var parts = line.split(" = ")
					steps.append({
						"lines": [16 + i * 3, 16 + i * 3 + 2],
						"explanation": "[b]Insert at Index[/b]\nInserts element at a specific position.\nExisting elements shift right.\n[color=green]Result: %s[/color]" % (parts[1] if parts.size() > 1 else "")
					})
				elif line.begins_with("After delete"):
					var parts = line.split(" = ")
					steps.append({
						"lines": [16 + i * 3, 16 + i * 3 + 2],
						"explanation": "[b]Delete at Index[/b]\nRemoves element at the given index.\nRemaining elements shift left.\n[color=red]Result: %s[/color]" % (parts[1] if parts.size() > 1 else "")
					})
				elif line.begins_with("Access"):
					steps.append({
						"lines": [16 + i * 3, 16 + i * 3 + 1],
						"explanation": "[b]Array Access[/b]\nReads the value at a given index.\nDoes not modify the array.\n[color=yellow]%s[/color]" % line
					})
		"python":
			steps.append({
				"lines": range(0, 3),
				"explanation": "[b]Function Definition[/b]\nDefines dfs() taking tree list and target.\nTime: O(V+E) | Space: O(V)"
			})
			steps.append({
				"lines": range(3, 6),
				"explanation": "[b]print_array Helper[/b]\nConverts list to comma-separated string and prints it."
			})
			steps.append({
				"lines": range(7, 9),
			...
			for i in range(code_lines.size()):
				var line = code_lines[i]
				if line.begins_with("Initial list"):
					steps.append({"lines": [9 + i * 3, 9 + i * 3 + 2], "explanation": "[b]Initial Array[/b]\n[color=yellow]%s[/color]" % line.split(" = ")[1]})
				elif line.begins_with("After insert") and "end" in line:
					steps.append({"lines": [9 + i * 3, 9 + i * 3 + 1], "explanation": "[b]Insert at End[/b]\narr.append() adds to back.\n[color=green]%s[/color]" % line.split(" = ")[1] if " = " in line else ""})
				elif line.begins_with("After insert"):
					steps.append({"lines": [9 + i * 3, 9 + i * 3 + 1], "explanation": "[b]Insert at Index[/b]\narr.insert(i, val) shifts elements right.\n[color=green]%s[/color]" % (line.split(" = ")[1] if " = " in line else "")})
				elif line.begins_with("After delete"):
					steps.append({"lines": [9 + i * 3, 9 + i * 3 + 1], "explanation": "[b]Delete[/b]\ndel arr[i] removes and shifts left.\n[color=red]%s[/color]" % (line.split(" = ")[1] if " = " in line else "")})
				elif line.begins_with("Access"):
					steps.append({"lines": [9 + i * 2, 9 + i * 2 + 1], "explanation": "[b]Access[/b]\nReads value at index.\n[color=yellow]%s[/color]" % line})
		"java", "c":
			# Generic structural steps for java/c
			steps.append({"lines": range(0, 3), "explanation": "[b]Imports & Setup[/b]\nImports necessary libraries."})
			steps.append({"lines": range(3, 10), "explanation": "[b]Print Function[/b]\nHelper to display array contents."})
			steps.append({"lines": range(10, 13), "explanation": "[b]Main & Init[/b]\nEntry point and array declaration."})
			for i in range(code_lines.size()):
				var line = code_lines[i]
				if line.begins_with("Initial list"):
					steps.append({"lines": [13 + i * 3, 13 + i * 3 + 2], "explanation": "[b]Initial Array[/b]\n[color=yellow]%s[/color]" % (line.split(" = ")[1] if " = " in line else line)})
				elif line.begins_with("After insert") and "end" in line:
					steps.append({"lines": [13 + i * 3, 13 + i * 3 + 1], "explanation": "[b]Insert at End[/b]\n[color=green]%s[/color]" % (line.split(" = ")[1] if " = " in line else "")})
				elif line.begins_with("After insert"):
					steps.append({"lines": [13 + i * 3, 13 + i * 3 + 1], "explanation": "[b]Insert at Index[/b]\n[color=green]%s[/color]" % (line.split(" = ")[1] if " = " in line else "")})
				elif line.begins_with("After delete"):
					steps.append({"lines": [13 + i * 3, 13 + i * 3 + 1], "explanation": "[b]Delete[/b]\n[color=red]%s[/color]" % (line.split(" = ")[1] if " = " in line else "")})
				elif line.begins_with("Access"):
					steps.append({"lines": [13 + i * 2, 13 + i * 2], "explanation": "[b]Access[/b]\n[color=yellow]%s[/color]" % line})
...
# FIX 2: Auto-scroll to highlighted line in _update_cpp_walkthrough
func _update_cpp_walkthrough():
	...
	# Update explanation - same as bubble sort
	if cpp_explanation_lbl:
		cpp_explanation_lbl.bbcode_enabled = true
	...
	if cpp_text:
		...
		# Build highlighted code - same pattern as bubble sort's _update_cpp_tutorial
		...
		# Scroll - exact same pattern as bubble sort
		if highlight_lines.size() > 0:
			var target_line = highlight_lines[0] if highlight_lines is Array else highlight_lines.min()
			await get_tree().process_frame
