...
# FIX 3: _build_spawn_stack — fix signal connection same way
...
# FIX 6: _on_block_pressed — now receives block directly from signal
...
# FIX 5: _on_spawn_block_dropped — now receives block directly from signal
...
# FIX 7: _get_drop_index — use actual block positions not hardcoded 64.0
# FIX 1: _get_drop_index — account for array_container global offset properly
...
# FIX 4: _on_array_block_dropped — now receives block directly from signal
...
# FIX 2: _animate_trash_eat — reset scale back to original after animation
...
# FIX 2: _rebuild_blocks_from_array — fix signal connection (remove .bind, 
# use index capture instead) and store original_position properly
# FIX 2: _rebuild_blocks_from_array — fix signal connection (remove .bind, 
# use index capture instead) and store original_position properly
func _rebuild_blocks_from_array() -> void:
	...
	for i in range(main_array.size()):
		...
		# FIX: Connect without .bind — the signal already passes self
