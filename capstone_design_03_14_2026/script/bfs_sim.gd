...
func _perform_bfs_step():
	...
	_add_code_line("DEQUEUE", curr_idx, val)
	...
	if val == target_value:
		...
		if left < 7 and tree_nodes[left] != null and main_array[left] != 0:
			...
			_add_code_line("ENQUEUE", left, main_array[left])
		...
		if right < 7 and tree_nodes[right] != null and main_array[right] != 0:
			...
			_add_code_line("ENQUEUE", right, main_array[right])
...
func _gen_cpp_code() -> String:
	var code = "/* BFS Tree Search Simulation - Operations Log */\n"
...
func _gen_c_code() -> String:
	...
	code += "void enqueue(int v) { queue[rear++] = v; }\n"
	code += "int dequeue() { return queue[front++]; }\n"
	code += "bool is_empty() { return front == rear; }\n\n"
	code += "void printTree(int tree[], int size) {\n"
...
func _show_result_popup(result: String, grade: Dictionary) -> void:
	...
	if result == "PASS":
		result_title.text = "TARGET FOUND!"
	...
	score_summary.text = "Nodes Visited: %d" % grade.get("correct_moves", 0)
	accuracy_label.text = "Target: %d" % target_value
	time_used_label.text = "Queue Size: %d" % bfs_queue.size()
